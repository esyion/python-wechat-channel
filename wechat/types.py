"""WeChat ilink protocol types — pydantic models mirroring src/wechat/types.ts."""
...
class CDNMedia(BaseModel):
    ...
    aes_key: Optional[str] = None  # base64-encoded bytes
...
class SendMessageReq(BaseModel):
    msg: Optional[dict] = None  # fully dynamic, built at call site
...
class UploadedFileInfo(BaseModel):
    ...
    aeskey: str  # Hex-encoded raw 16-byte AES key
    file_size: int  # Plaintext file size in bytes
    file_size_ciphertext: int  # Ciphertext file size (AES-128-ECB + PKCS7)
...
class GetBotQrcodeResp(BaseModel):
    qrcode: str
    qrcode_img_content: str


class GetQrcodeStatusResp(BaseModel):
    status: str
